...
export class ProfileComponent implements OnInit, OnDestroy {
			...
			this.subscriptions.push(
				this.userService.getUserById(this.profileUserId).subscribe({
					next: (foundUserResponse: UserResponse) => {
						...
						this.profileImge='data:image/jpeg;base64,'+foundUser.pimg;
						this.coverImge='data:image/jpeg;base64,'+foundUser.cimg;
				...
				this.userService.getUserPosts(this.profileUserId, currentPage, this.resultSize).subscribe({
					next: (postResponses: PostResponse[]) => {
						postResponses.forEach(post => {
							post.post.img='data:image/jpeg;base64,'+post.post.postPhotoByte
	...
	openChat(userId:number){
		this.matDialog.open(ChatPopupComponent,{
			// height:'600px',
			width:'500px',
			data:userId
		})
	...
	getFriendsList(){
		this.userService.getUserFollowerList( this.authUser.id,1,10).subscribe({
			next:(res:UserResponse[])=>{
				res.forEach(uR =>{
					{
						uR.user.profilePhoto='data:image/jpeg;base64,'+uR.user.pimg;
						this.friendList.push(uR);
					}
				});
			}
		})
		// this.userService.getUserFollowingList( this.authUser.id,1,10).subscribe({
		// 	next:(res:UserResponse[])=>{
		// 		res.forEach(uR =>{
		// 			{
		// 				uR.user.profilePhoto='data:image/jpeg;base64,'+uR.user.pimg;
